...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (rc == NGX_AGAIN) {
		...
		g_error(NGX_LOG_ERR, r->connection->log, 0, "no live upstreams");
		        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_NOLIVE);
				        return;
        return;
    }
...
i		//难道此间会发生什么使得不再event-ready吗？很多地方都有这样一个检查
        if (!rev->ready) {
            break;
        }
    }
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (!u->buffering) {
        ...
        if (n) {
            ...
            u->state->response_length += n;
			//参见nob_bufferd_filter中pos和last的处理
            if (u->input_filter(u->input_filter_ctx, n) == NGX_ERROR) {
                ngx_http_upstream_finalize_request(r, u, 0);
                return;
            }
    ...
    ngx_uint_t do_write)
{
    ...
    for ( ;; ) {
        ...
        if (do_write) {
            ...
            if (u->busy_bufs == NULL) {//发完了

                if (u->length == 0//且不需再接受包体
                    || upstream->read->eof
                    || upstream->read->error)
                {
                    ngx_http_upstream_finalize_request(r, u, 0);
