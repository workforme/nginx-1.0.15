...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
			...
			//注意所在函数叫ngx_http_process_request_headers差一个s
...
//keepalive也不是无限等待哦，也会超时
    if (rev->timedout || c->close) {
		//而不是close request，因为二次进入的都是尚未填充request
        ngx_http_close_connection(c);
        return;
    }
...
/*
linger本身都是为了等server尽量写完数据到client，但是下面两个是从read超时来实现，server要等多久才关链接的。
 lingering_timeout之内收到数据则半关闭写端（否则彻底关闭），然后lingering_time决定为read服务再多久。
lingering_time:
